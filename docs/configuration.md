...
The canonical user-facing pipeline format is:
...
Use this format for both starter pipelines generated by `flowctl init` and custom pipelines you build yourself.

Key points:
- `flowctl/v1` is the current format to learn and document
- starter pipelines generated by `flowctl init` are regular `flowctl/v1` pipelines
- run pipelines with: `flowctl run pipeline.yaml`
- validate pipelines with: `flowctl validate pipeline.yaml`
...
If you want the fastest supported path, generate a starter pipeline instead of hand-writing one:
...
Starter examples also live under `examples/quickstart/`, but they are still standard `flowctl/v1` pipelines.
...
- **Starter pipelines:** `examples/quickstart/*.yaml`
