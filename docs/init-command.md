...
- **Default organization for short refs**: `withobsrvr`
- **Cache Location**: `~/.flowctl/components/`

Starter pipelines generated by `flowctl init` use short refs like:
...
If you want to reference a component from a different Docker Hub organization in your own pipeline, specify the org explicitly:

```yaml
type: myorg/my-component@v1.2.3
```

That resolves to:

```text
docker.io/myorg/my-component:v1.2.3
```
